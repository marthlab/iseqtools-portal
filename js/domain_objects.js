...
function Task(cfg) {
	_(this).extend(_(cfg).pickStrings('id', 'name'));
	this.in_data_types = cfg.in_data_types_ids.map(function(dt_id){ return _(app.data.data_types).find(by_id(dt_id));});
	this.out_data_types = cfg.out_data_types.map(function(dt_cfg){ return _(app.data.data_types).find(by_id(dt_cfg.id));});
...
function Workflow(cfg) {
	...
	this.tasks = cfg.tasks_ids.map(function(task_id){ return _(app.data.tasks).find(by_id(task_id));});
...
function DataFormat(cfg) {
	...
	//this.data_type = _(app.data.tasks).find(by_id(cfg.data_type_id)) || null;
}

function Tool(cfg) {
	_(this).extend(_(cfg).pickStrings('id'));
	this.name = cfg.name || cfg.id;
	this.team = _(app.data.teams).find(by_id(cfg.team_id)) || null;
}
...
function Pipeline(cfg) {
	...
	this.team = _(app.data.teams).find(by_id(cfg.team_id)) || null;
	this.workflow = _(app.data.workflows).find(by_id(cfg.workflow_id));
	this.data_types = cfg.data_types_ids.map(function(dt_id){ return _(app.data.data_types).find(by_id(dt_id));});
...
function DataFormatUsage(cfg) {
	...
	this.data_format = _(app.data.data_formats).find(by_id(cfg.data_format_id));
}

function ToolUsage(cfg) {
	_(this).extend(_(cfg).pickStrings('id'));
	this.pipeline = cfg.pipeline;
	this.tool = _(app.data.tools).find(by_id(cfg.tool_id));
	this.task = _(app.data.tasks).find(by_id(cfg.task_id)) || null;
